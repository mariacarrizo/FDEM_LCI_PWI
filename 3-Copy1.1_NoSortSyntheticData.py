...
for i in range(-1,39):
    for j in range(-1,39):
        dat = np.hstack((DATA_sorted.loc[(DATA_sorted.src_x > p+.06) & (DATA_sorted.src_x <=p +1.06) & DATA_sorted]['op'],
...
np.save('data/data_s'+sl+'_c'+co+'_u'+cr+'_nosort', DATA_sorted_source)
