...
for pos in range(npos):
    ...
    model_inv.append(inv.run(dataVals=dat, errorVals=rel_err, startModel=startModel, verbose=False, lam=10 ))
    response.append(inv.response)
    
# Save estimated model

np.save('results/model_inv_d'+sl+'_c'+co+'_u'+cr+'_1D_nosort_lam1', model_inv)
np.save('results/data_inv_d'+sl+'_c'+co+'_u'+cr+'_1D_nosort_lam1', response)
